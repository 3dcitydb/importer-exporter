...
public class GeneralPanel extends InternalPreferencesComponent {
    ...
    public boolean isModified() {
        ...
        try {
            groupSizeText.commitEdit();
        } catch (ParseException ignored) {
        }
        try {
            visibleFromText.commitEdit();
        } catch (ParseException ignored) {
        }
        try {
            viewRefreshTimeText.commitEdit();
        } catch (ParseException ignored) {
        }
        ...
        if (packingAlgorithms.get(packingAlgorithmsComboBox.getSelectedItem()) != colladaOptions.getPackingAlgorithm())
            return true;
        ...
        if (((Number) visibleFromText.getValue()).intValue() != visExportConfig.getSingleObjectRegionSize())
            return true;
        if (((Number) viewRefreshTimeText.getValue()).doubleValue() != visExportConfig.getViewRefreshTime())
            return true;
