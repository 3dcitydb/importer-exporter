...
public class SAXSplitter extends XMLFilterImpl {	
	...
	protected final SAXBuffer saxBuffer;
	...
	public SAXSplitter(WorkerPool<Vector<SAXEvent>> threadPool, Config config, EventDispatcher eventDispatcher) {
		...
		if (isCityGMLElement && localName.equals("cityObjectMember")) {
			...
			if (cityObjectChildElement != null) {
				if (type == CityGMLClass.CITYOBJECTGROUP) {					
					...
					if (locator != null)
						location = new DocumentLocation(locator.getLineNumber(), locator.getColumnNumber());

					StartElement groupMemberElement = new StartElement(
	...
	private final class CityObjectChildElement {
		private String uri;
