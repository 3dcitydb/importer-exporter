...
import de.tub.citygml4j.model.citygml.transportation.AuxiliaryTrafficArea;
import de.tub.citygml4j.model.citygml.transportation.AuxiliaryTrafficAreaProperty;
import de.tub.citygml4j.model.citygml.transportation.TrafficArea;
...
import de.tub.citygml4j.model.citygml.transportation.TransportationObject;
...
public class DBTransportationComplex implements DBExporter {
	...
	private TransportationModule transFactory;
	...
	private void init() throws SQLException {
		...
		transFactory = config.getProject().getExporter().getModuleVersion().getTransportation().getModule();

		psTransComplex = connection.prepareStatement("select tc.ID as TC_ID, tc.NAME as TC_NAME, tc.NAME_CODESPACE as TC_NAME_CODESPACE, tc.DESCRIPTION as TC_DESCRIPTION, tc.FUNCTION as TC_FUNCTION, tc.USAGE as TC_USAGE, " +
	...
	public boolean read(DBSplittingResult splitter) throws SQLException, JAXBException {
		...
		case ROAD:
			transComplex = cityGMLFactory.createRoad(transFactory);
			break;
		case RAILWAY:
			transComplex = cityGMLFactory.createRailway(transFactory);
			break;
		case SQUARE:
			transComplex = cityGMLFactory.createSquare(transFactory);
			break;
		case TRACK:
			transComplex = cityGMLFactory.createTrack(transFactory);
			break;
		default:
			transComplex = cityGMLFactory.createTransportationComplex(transFactory);
		}
		...
		rs = psTransComplex.executeQuery();
		...
		while (rs.next()) {
			...
			if (isAuxiliary)
				transObject = cityGMLFactory.createAuxiliaryTrafficArea(transFactory);
			else
				transObject = cityGMLFactory.createTrafficArea(transFactory);
			...
			if (isAuxiliary) {
				AuxiliaryTrafficAreaProperty auxProperty  = cityGMLFactory.createAuxiliaryTrafficAreaProperty(transFactory);
				auxProperty.setObject((AuxiliaryTrafficArea)transObject);
				transComplex.addAuxiliaryTrafficArea(auxProperty);
			} else {
				TrafficAreaProperty trafficProperty = cityGMLFactory.createTrafficAreaProperty(transFactory);
				trafficProperty.setObject((TrafficArea)transObject);
				transComplex.addTrafficArea(trafficProperty);
			}
		}

		if (transComplex.getId() != null && !featureClassFilter.filter(CityGMLClass.CITYOBJECTGROUP))
