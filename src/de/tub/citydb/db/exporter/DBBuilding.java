...
import de.tub.citygml4j.implementation.gml._3_1_1.MeasureOrNullListImpl;
...
import de.tub.citygml4j.model.citygml.building.BuildingPart;
...
public class DBBuilding implements DBExporter {
	...
	private BuildingModule bldgFactory;
	...
	private void init() throws SQLException {
		...
		bldgFactory = config.getProject().getExporter().getModuleVersion().getBuilding().getModule();
		coreFactory = bldgFactory.getCoreDependency();
		...
		while (rs.next()) {
			...
			if (!rs.wasNull() && addressId > 0) {
				...
				if (countryAttr != null) {
					...
					XalAddressProperty xalAddressProperty = cityGMLFactory.createXalAddressProperty(coreFactory);
					xalAddressProperty.setAddressDetails(addressDetails);

					Address address = cityGMLFactory.createAddress(coreFactory);
					...
					addressProperty = cityGMLFactory.createAddressProperty(coreFactory);
	...
	private AbstractBuilding rebuildBuilding(BuildingNode buildingNode) throws SQLException {
		...
		if (buildingNode.parentId != 0) {
			...
			abstractBuilding = cityGMLFactory.createBuildingPart(bldgFactory);
		} else {
			abstractBuilding = cityGMLFactory.createBuilding(bldgFactory);
		}

		// do cityObject stuff
		boolean success = cityObjectExporter.read(abstractBuilding, buildingNode.id, true);
		if (!success)
			return null;
		...
		if (buildingNode.yearOfConstruction != null) {
			...
			abstractBuilding.setYearOfConstruction(xmlDate);
		}
		...
		if (buildingNode.yearOfDemolition != null) {
			...
			XMLGregorianCalendar xmlDate = new XMLGregorianCalendarImpl(gregDate);
			abstractBuilding.setYearOfDemolition(xmlDate);
		}
		...
		for (int lod = 1; lod < 5; lod++) {
			...
			if (terrainIntersection != null) {
				...
				if (multiCurveProperty != null) {
					switch (lod) {
					case 1:
						abstractBuilding.setLod1TerainIntersection(multiCurveProperty);
						break;
					case 2:
						abstractBuilding.setLod2TerainIntersection(multiCurveProperty);
						break;
					case 3:
						abstractBuilding.setLod3TerainIntersection(multiCurveProperty);
						break;
					case 4:
						abstractBuilding.setLod4TerainIntersection(multiCurveProperty);
						break;
					}
				}
			}
		...
		// BoundarySurface
		thematicSurfaceExporter.read(abstractBuilding, buildingNode.id, bldgFactory);

		// BuildingInstallation
		buildingInstallationExporter.read(abstractBuilding, buildingNode.id, bldgFactory);

		// room
		roomExporter.read(abstractBuilding, buildingNode.id, bldgFactory);
		...
		for (BuildingNode childNode : buildingNode.childNodes) {
			...
			BuildingPartProperty buildingPartProperty = cityGMLFactory.createBuildingPartProperty(bldgFactory);
			buildingPartProperty.setObject(buildingPart);
			abstractBuilding.addConsistsOfBuildingPart(buildingPartProperty);
		}
		
		if (abstractBuilding.getId() != null && !featureClassFilter.filter(CityGMLClass.CITYOBJECTGROUP))
