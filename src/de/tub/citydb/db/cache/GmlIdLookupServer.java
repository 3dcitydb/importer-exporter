...
public class GmlIdLookupServer {
	...
	private void drainToDB() {
		try {
			LOG.debug("Writing gml:id cache to database.");
			...
			try {
				...
				LOG.debug("gml:id cache written to database.");

			} catch (SQLException sqlEx) {
				LOG.error("SQL error while writing gml:id cache to database: " + sqlEx.getMessage());
			}
		} finally {
