...
public class DBWaterBody implements DBImporter {
	...
	private boolean insert(WaterBody waterBody, long waterBodyId) throws SQLException {
		...
		// gml:description
		if (waterBody.getDescription() != null) {
		...
		if (waterBody.getFunction() != null) {
			List<String> functionList = waterBody.getFunction();
			psWaterBody.setString(6, Util.collection2string(functionList, " "));
		} else {
		...
		if (waterBody.getUsage() != null) {
			List<String> usageList = waterBody.getUsage();
			psWaterBody.setString(7, Util.collection2string(usageList, " "));
		} else {
		...
		// boundary surfaces
		List<BoundedByWaterSurfaceProperty> boundedByList = waterBody.getBoundedBySurfaces();
		if (boundedByList != null) {
			for (BoundedByWaterSurfaceProperty waterSurfaceProperty : boundedByList) {
				if (waterSurfaceProperty.getObject() != null) {
					long id = boundarySurfaceImporter.insert(waterSurfaceProperty.getObject(), waterBodyId);
					if (id == 0)
						System.out.println("Could not write WaterBoundarySurface");
				} else {
					// xlink
