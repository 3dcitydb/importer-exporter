	...
	private boolean insert(AbstractBuilding building,
		...
		if (building.isSetBoundedBySurfaces()) {
			for (BoundarySurfaceProperty boundarySurfaceProperty : building.getBoundedBySurfaces()) {
				if (boundarySurfaceProperty.isSetObject()) {
					String gmlId = boundarySurfaceProperty.getObject().getId();
					...
					if (id == 0) {
						...
						msg.append(Util.getFeatureSignature(
								boundarySurfaceProperty.getObject().getCityGMLClass(), 
								gmlId));
		...
		if (building.isSetOuterBuildingInstallation()) {
			for (BuildingInstallationProperty buildingInstProperty : building.getOuterBuildingInstallation()) {
				if (buildingInstProperty.isSetObject()) {
					String gmlId = buildingInstProperty.getObject().getId();
		...
		if (building.isSetInteriorBuildingInstallation()) {
			for (IntBuildingInstallationProperty intBuildingInstProperty : building.getInteriorBuildingInstallation()) {
				if (intBuildingInstProperty.isSetObject()) {
					String gmlId = intBuildingInstProperty.getObject().getId();
		...
		if (building.isSetInteriorRoom()) {
			for (InteriorRoomProperty roomProperty : building.getInteriorRoom()) {
				if (roomProperty.isSetObject()) {
					String gmlId = roomProperty.getObject().getId();
				...
				if (buildingPartProperty.isSetObject()) {
					...
					if (id != 0)
						...
						msg.append(Util.getFeatureSignature(
								...
								buildingPartProperty.getObject().getId()));
		...
		if (building.isSetAddress()) {
			for (AddressProperty addressProperty : building.getAddress()) {
				if (addressProperty.isSetObject()) {
					String gmlId = addressProperty.getObject().getId();
