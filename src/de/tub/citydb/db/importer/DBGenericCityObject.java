...
public class DBGenericCityObject implements DBImporter {
	...
	private boolean insert(GenericCityObject genericCityObject, long genericCityObjectId) throws SQLException {
		// CityObject
		long cityObjectId = cityObjectImporter.insert(genericCityObject, genericCityObjectId);
		if (cityObjectId == 0)
			return false;
		...
		// gml:name
		if (genericCityObject.getName() != null) {
		...
		// gml:description
		if (genericCityObject.getDescription() != null) {
		...
		// citygml:class
		if (genericCityObject.getClazz() != null)
		...
		if (genericCityObject.getFunction() != null) {
			List<String> functionList = genericCityObject.getFunction();
			psGenericCityObject.setString(6, Util.collection2string(functionList, " "));
		} else {
		...
		if (genericCityObject.getUsage() != null) {
			List<String> usageList = genericCityObject.getUsage();
			psGenericCityObject.setString(7, Util.collection2string(usageList, " "));
		} else {
    			...
    			if (geometryProperty.getGeometry() != null) {
        			...
        			if (href != null && href.length() != 0) {
        				DBXlinkBasic xlink = new DBXlinkBasic(
        						genericCityObjectId,
        						DBTableEnum.GENERIC_CITYOBJECT,
        						href,
		...
		for (int lod = 0; lod < 5; lod++) {
			...
			switch (lod) {
			case 0:
				multiCurveProperty = genericCityObject.getLod0TerainIntersection();
				break;
			case 1:
				multiCurveProperty = genericCityObject.getLod1TerainIntersection();
				break;
			case 2:
				multiCurveProperty = genericCityObject.getLod2TerainIntersection();
				break;
			case 3:
				multiCurveProperty = genericCityObject.getLod3TerainIntersection();
				break;
			case 4:
				multiCurveProperty = genericCityObject.getLod4TerainIntersection();
				break;
			}
