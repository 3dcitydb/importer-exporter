...
public class DBOpening implements DBImporter {
	...
	public long insert(Opening opening, long parentId) throws SQLException {
		...
		// gml:description
		if (opening.getDescription() != null) {
		...
		if (opening.getCityGMLClass() == CityGMLClass.DOOR) {
			List<AddressProperty> addressPropertyList = ((Door)opening).getAddress();
			long addressId = 0;
			
			if (addressPropertyList != null) {
				// unfortunately, we can just handle represent one address in database...				
				AddressProperty addressProperty = addressPropertyList.get(0);

				if (addressProperty.getObject() != null) {
					addressId = addressImporter.insert(addressProperty.getObject());
				} else {
					// xlink
