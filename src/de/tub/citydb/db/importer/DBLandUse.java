...
public class DBLandUse implements DBImporter {
	...
	private boolean insert(LandUse landUse, long landUseId) throws SQLException {
		...
		// gml:description
		if (landUse.getDescription() != null) {
		...
		if (landUse.getFunction() != null) {
			List<String> functionList = landUse.getFunction();
			psLandUse.setString(6, Util.collection2string(functionList, " "));
		} else {
		...
		if (landUse.getUsage() != null) {
			List<String> usageList = landUse.getUsage();
			psLandUse.setString(7, Util.collection2string(usageList, " "));
		} else {
