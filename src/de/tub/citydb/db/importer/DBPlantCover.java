...
public class DBPlantCover implements DBImporter {
	...
	private boolean insert(PlantCover plantCover, long plantCoverId) throws SQLException {
		...
		// gml:description
		if (plantCover.getDescription() != null) {
		...
		if (plantCover.getFunction() != null) {
			List<String> functionList = plantCover.getFunction();
			psPlantCover.setString(6, Util.collection2string(functionList, " "));
		} else {
		...
		if (plantCover.getAverageHeight() != null) {
			Length averageHeight= plantCover.getAverageHeight();
			psPlantCover.setDouble(7, averageHeight.getValue());
		} else {
		...
		for (int lod = 1; lod < 5; lod++) {
			...
			if (multiSolidProperty != null) {
				if (multiSolidProperty.getMultiSolid() != null) {
