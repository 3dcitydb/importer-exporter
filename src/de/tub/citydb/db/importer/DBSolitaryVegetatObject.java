...
public class DBSolitaryVegetatObject implements DBImporter {
	...
	private boolean insert(SolitaryVegetationObject solVegObject, long solVegObjectId) throws SQLException {
		...
		// gml:description
		if (solVegObject.getDescription() != null) {
		...
		// citygml:class
		if (solVegObject.getClazz() != null)
			psSolitVegObject.setString(5, solVegObject.getClazz().trim());
		else
			psSolitVegObject.setNull(5, Types.VARCHAR);

		// species
		if (solVegObject.getSpecies() != null)
			psSolitVegObject.setString(6, solVegObject.getSpecies());
		else
			psSolitVegObject.setNull(6, Types.VARCHAR);

		// citygml:function
		if (solVegObject.getFunction() != null) {
			List<String> functionList = solVegObject.getFunction();
			psSolitVegObject.setString(7, Util.collection2string(functionList, " "));
		} else {
			psSolitVegObject.setNull(7, Types.VARCHAR);
		}

		// height
		if (solVegObject.getHeight() != null) {
			Length length = solVegObject.getHeight();
			psSolitVegObject.setDouble(8, length.getValue());
		} else {
			psSolitVegObject.setNull(8, Types.DOUBLE);
		}

		// trunc diameter
		if (solVegObject.getTrunkDiameter() != null) {
			Length length= solVegObject.getTrunkDiameter();
			psSolitVegObject.setDouble(9, length.getValue());
		} else {
			psSolitVegObject.setNull(9, Types.DOUBLE);
		}

		// crown diameter
		if (solVegObject.getCrownDiameter() != null) {
			Length length= solVegObject.getCrownDiameter();
			psSolitVegObject.setDouble(10, length.getValue());
		} else {
