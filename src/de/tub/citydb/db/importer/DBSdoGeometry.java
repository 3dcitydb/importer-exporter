		...
		if (polygonProperty != null && polygonProperty.isSetPolygon()) {
			...
			if (polygon.isSetExterior()) {
				...
				if (exteriorLinearRing != null) {
					...
					if (points != null && !points.isEmpty()) {
						if (!points.get(0).equals(points.get(points.size() - 3))) {
						...
						if (polygon.getInterior() != null) {
							...
							for (AbstractRingProperty abstractRingProperty : abstractRingPropertyList) {
								...
								if (interiorPoints != null && !interiorPoints.isEmpty()) {
									if (!interiorPoints.get(0).equals(interiorPoints.get(interiorPoints.size() - 3))) {
										// repair unclosed ring because sdoapi fails to do its job...
										points.add(interiorPoints.get(0));
										points.add(interiorPoints.get(1));
										points.add(interiorPoints.get(2));
									}
				...
				if (!pointList.isEmpty()) {
					...
					polygonGeom = JGeometry.createLinearPolygon(pointArray, dim, Integer.valueOf(dbSrid));
				}
			}
		}
