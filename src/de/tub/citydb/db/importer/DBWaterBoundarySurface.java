...
public class DBWaterBoundarySurface implements DBImporter {
	...
	public long insert(WaterBoundarySurface waterBoundarySurface, long parentId) throws SQLException {
		...
		// gml:name
    	if (waterBoundarySurface.getName() != null) {
		...
		// gml:description
		if (waterBoundarySurface.getDescription() != null) {
		...
		if (waterBoundarySurface.getCityGMLClass() == CityGMLClass.WATERSURFACE) {
			String waterLevel = ((WaterSurface)waterBoundarySurface).getWaterLevel();
			if (waterLevel != null && waterLevel.length() != 0)
				psWaterBoundarySurface.setString(6, waterLevel);
			else
