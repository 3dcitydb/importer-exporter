...
public class DBThematicSurface implements DBImporter {
	...
	public long insert(BoundarySurface boundarySurface, CityGMLClass parent, long parentId) throws SQLException {
		...
		// gml:name
		if (boundarySurface.getName() != null) {
		...
		// gml:description
		if (boundarySurface.getDescription() != null) {
        ...
        // Openings
        List<OpeningProperty> openingPropertyList = boundarySurface.getOpening();
        if (openingPropertyList != null) {
        	for (OpeningProperty openingProperty : openingPropertyList) {
        		if (openingProperty.getObject() != null) {
        			long id = openingImporter.insert(openingProperty.getObject(), boundarySurfaceId);
        			if (id == 0)
        				System.out.println("Could not write Opening");
        		} else {
        			// xlink
