...
public class DBReliefComponent implements DBImporter {
	...
	public long insert(ReliefComponent reliefComponent, long parentId) throws SQLException {
		...
		// gml:name
		if (reliefComponent.getName() != null) {
		...
		// gml:description
		if (reliefComponent.getDescription() != null) {
		...
		if (reliefComponent.getExtent() != null) {
			JGeometry extent = sdoGeometry.getPolygon(reliefComponent.getExtent());
			if (extent != null) {
		...
		if (reliefComponent.getCityGMLClass() == CityGMLClass.TINRELIEF) {
			...
			if (tinRelief.getTin() != null) {
				...
				if (tinProperty.getObject() != null) {
					geometryId = surfaceGeometryImporter.insert(tinRelief.getTin().getObject(), reliefComponentId);
			
					// gml:Tin
					if (tinProperty.getObject().getGMLClass() == GMLClass.TIN) {
						Tin tin = (Tin)tinProperty.getObject();
						
						// maxLength
						if (tin.getMaxLength() != null) 
							maxLength = tin.getMaxLength().getValue();
						
						// stopLines
						if (tin.getStopLines() != null)
						...
						// controlPoints
						if (tin.getControlPoint() != null)
							controlPoints = sdoGeometry.getMultiPoint(tin.getControlPoint());
					}
					
				} else {
					// xlink
					System.out.println("xlinks on TINRelief are not supported");
				}
			}
