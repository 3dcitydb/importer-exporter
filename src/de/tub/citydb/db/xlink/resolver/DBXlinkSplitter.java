...
public class DBXlinkSplitter {
	...
	private void basicXlinks() throws SQLException {
		...
		try {
			...
			eventDispatcher.triggerEvent(new StatusDialogProgressBar(0, 0));
			eventDispatcher.triggerEvent(new StatusDialogMessage(ImpExpGui.labels.getString("import.dialog.basicXLink.msg")));
			...
			while (rs.next() && shouldRun) {
				eventDispatcher.triggerEvent(new StatusDialogProgressBar(++current, max));
	...
	private void queryGroupMemberXLinks(TemporaryCacheTable cacheTable, 
		...
		try {					
			eventDispatcher.triggerEvent(new StatusDialogProgressBar(0, 0));
			String text = ImpExpGui.labels.getString("import.dialog.groupXLink.msg");
	...
	private void appearanceXlinks() throws SQLException {
		...
		try {
			...
			eventDispatcher.triggerEvent(new StatusDialogProgressBar(0, 0));
			eventDispatcher.triggerEvent(new StatusDialogMessage(ImpExpGui.labels.getString("import.dialog.appXlink.msg")));
			...
			if (cacheManager.existsTemporaryCacheTable(CacheTableModelEnum.TEXTURE_FILE)) {		
				...
				eventDispatcher.triggerEvent(new StatusDialogProgressBar(0, 0));
				eventDispatcher.triggerEvent(new StatusDialogMessage(ImpExpGui.labels.getString("import.dialog.texImg.msg")));
	...
	private void libraryObjectXLinks() throws SQLException {
		...
		try {
			...
			eventDispatcher.triggerEvent(new StatusDialogProgressBar(0, 0));
			eventDispatcher.triggerEvent(new StatusDialogMessage(ImpExpGui.labels.getString("import.dialog.libObj.msg")));
	...
	private void deprecatedMaterialXlinks() throws SQLException {
		...
		try {
			...
			eventDispatcher.triggerEvent(new StatusDialogProgressBar(0, 0));
			eventDispatcher.triggerEvent(new StatusDialogMessage(ImpExpGui.labels.getString("import.dialog.depMat.msg")));
	...
	private void querySurfaceGeometryXlinks(TemporaryCacheTable cacheTable, 
		...
		try {
			eventDispatcher.triggerEvent(new StatusDialogProgressBar(0, 0));
			String text = ImpExpGui.labels.getString("import.dialog.geomXLink.msg");
			Object[] args = new Object[]{ pass };
			eventDispatcher.triggerEvent(new StatusDialogMessage(MessageFormat.format(text, args)));
