...
public class XlinkBasic implements DBXlinkResolver {
	...
	public XlinkBasic(Connection batchConn, Connection commitConn, DBXlinkResolverManager resolverManager) throws SQLException {
		this.batchConn = batchConn;
		this.commitConn = commitConn;
		this.resolverManager = resolverManager;

		psMap = new HashMap<String, PreparedStatement>();
		psUpdateSurfGeom = commitConn.prepareStatement("update SURFACE_GEOMETRY set IS_XLINK=1 where ID=? and IS_XLINK=0");
	}
	...
	public boolean insert(DBXlinkBasic xlink) throws SQLException {
		...
		if (xlink.getToTable() == DBTableEnum.SURFACE_GEOMETRY ||
				(xlink.getFromTable() == DBTableEnum.IMPLICIT_GEOMETRY && xlink.getToTable() == DBTableEnum.IMPLICIT_GEOMETRY)) {
			psUpdateSurfGeom.setLong(1, entry.getId());
			psUpdateSurfGeom.execute();
			commitConn.commit();
		}
