...
public class XlinkSurfaceGeometry implements DBXlinkResolver {
	...
	public XlinkSurfaceGeometry(Connection batchConn, DBTempHeapTable heapView, Config config, DBXlinkResolverManager resolverManager) throws SQLException {
	...
	private void init() throws SQLException {
		...
		psSelectTmpSurfGeom = heapView.getConnection().prepareStatement("select ID from " + heapView.getTableName() + " where PARENT_ID=?");
	...
	private void rebuildGeometry(GeometryNode geomNode, boolean reverse) {
		if (geomNode.geometry != null) {
			...
			if (!geomNode.isSolid && geomNode.isComposite) {
				...
				// differ between multiPolygon, multiSurface and multiSolid
				if (geomNode.childNodes == null || geomNode.childNodes.size() == 0)
					return;
				...
				if (isMultiSolid) {
					...
					// we do not further differ between multiPolygon or multiSurface...
					// everything will be a multiSurface
