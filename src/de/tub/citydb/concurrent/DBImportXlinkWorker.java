...
import de.tub.citydb.db.xlink.importer.DBXlinkImporterExternalFile;
...
public class DBImportXlinkWorker implements Worker<DBXlink> {
	...
	public DBImportXlinkWorker(DBTempTableManager dbTempTableManager, Config config, EventDispatcher eventDispatcher) {
		this.config = config;
		...
		try {
			try {
				...
				case TEXTUREASSOCIATION:
					...
					break;
				case EXTERNAL_FILE:
					DBXlinkExternalFile xlinkFile = (DBXlinkExternalFile)work;

					DBXlinkImporterExternalFile dbFile = (DBXlinkImporterExternalFile)dbXlinkManager.getDBImporterXlink(DBXlinkImporterEnum.EXTERNAL_FILE);
					if (dbFile != null)
