...
public class DBBridgeRoom implements DBImporter {
	...
	private DBBridgeFurniture buildingFurnitureImporter;
	private DBBridgeInstallation buildingInstallationImporter;
	...
	private void init() throws SQLException {
		...
		buildingFurnitureImporter = (DBBridgeFurniture)dbImporterManager.getDBImporter(DBImporterEnum.BRIDGE_FURNITURE);
		buildingInstallationImporter = (DBBridgeInstallation)dbImporterManager.getDBImporter(DBImporterEnum.BRIDGE_INSTALLATION);
	}

	public long insert(BridgeRoom bridgeRoom, long buildingId) throws SQLException {
		...
		if (bridgeRoom.isSetBridgeRoomInstallation()) {
			for (IntBridgeInstallationProperty intBridgeInstProperty : bridgeRoom.getBridgeRoomInstallation()) {
				...
				if (intBrigdeInst != null) {
					...
					long id = buildingInstallationImporter.insert(intBrigdeInst, bridgeRoom.getCityGMLClass(), bridgeRoomId);
		...
		if (bridgeRoom.isSetInteriorFurniture()) {
			for (InteriorFurnitureProperty intFurnitureProperty : bridgeRoom.getInteriorFurniture()) {
				...
				if (furniture != null) {
					...
					long id = buildingFurnitureImporter.insert(furniture, bridgeRoomId);
