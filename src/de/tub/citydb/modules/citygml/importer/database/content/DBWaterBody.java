...
public class DBWaterBody implements DBImporter {
	...
	private boolean insert(WaterBody waterBody, long waterBodyId) throws SQLException {
		...
		for (int lod = 0; lod < 2; lod++) {
			...
			case 0:
				if (multiLine != null) {
					psWaterBody.setObject(14, multiLine);
				} else
					psWaterBody.setNull(14, Types.OTHER, "ST_GEOMETRY");
					
				break;
			case 1:
				if (multiLine != null) {
					psWaterBody.setObject(15, multiLine);
				} else
					psWaterBody.setNull(15, Types.OTHER, "ST_GEOMETRY");
