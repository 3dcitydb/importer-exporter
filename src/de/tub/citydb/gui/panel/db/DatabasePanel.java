		...
		try {
			...
			if (!intConfig.isDbIsConnected()) {
				...
				if (dbConnection.getUser().trim().equals("")) {
					topFrame.errorMessage(ImpExpGui.labels.getString("db.dialog.error.conn.title"),
							ImpExpGui.labels.getString("db.dialog.error.conn.user"));
					return;
				}
				
				if (intConfig.getCurrentDbPassword().trim().equals("")) {
					topFrame.errorMessage(ImpExpGui.labels.getString("db.dialog.error.conn.title"),
							ImpExpGui.labels.getString("db.dialog.error.conn.pass"));
					return;
				}
				
				if (dbConnection.getServer().trim().equals("")) {
					topFrame.errorMessage(ImpExpGui.labels.getString("db.dialog.error.conn.title"),
							ImpExpGui.labels.getString("db.dialog.error.conn.server"));
					return;
				}

				if (dbConnection.getPort() == null) {
					topFrame.errorMessage(ImpExpGui.labels.getString("db.dialog.error.conn.title"),
							ImpExpGui.labels.getString("db.dialog.error.conn.port"));
					return;
				}

				if (dbConnection.getSid().trim().equals("")) {
					topFrame.errorMessage(ImpExpGui.labels.getString("db.dialog.error.conn.title"),
							ImpExpGui.labels.getString("db.dialog.error.conn.sid"));
					return;
				}			
				...
				try {
					...
					String text = ImpExpGui.labels.getString("db.dialog.error.openConn");
				...
				try {
					...
					String text = ImpExpGui.labels.getString("db.dialog.error.closeConn");
					Object[] args = new Object[]{ sqlEx.getMessage() };
					String result = MessageFormat.format(text, args);
	...
	private void report() {
		...
		try {
			...
			try {
				...
				String text = ImpExpGui.labels.getString("db.dialog.error.report.generate");
				Object[] args = new Object[]{ sqlEx.getMessage() };
				String result = MessageFormat.format(text, args);

				topFrame.errorMessage(ImpExpGui.labels.getString("common.dialog.error.db.title"), result);
				LOG.error("SQL error: " + sqlEx.getMessage().trim());
				return;
			}
			
			final StatusDialog reportDialog = new StatusDialog(topFrame, 
					ImpExpGui.labels.getString("db.dialog.report.title"), 
					ImpExpGui.labels.getString("db.dialog.report.msg"), 
					ImpExpGui.labels.getString("db.dialog.report.details"), true);
			...
			reportDialog.getButton().addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					SwingUtilities.invokeLater(new Runnable() {
						public void run() {
							reportDialog.getHeaderLabel().setText(ImpExpGui.labels.getString("common.dialog.msg.abort"));							
