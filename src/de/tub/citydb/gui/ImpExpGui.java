...
import de.tub.citydb.gui.components.ImpExpStatusDialog;
...
public class ImpExpGui extends JFrame {
	...
	public void doTranslation () {
		try {
			...
			menu.setTitleAt(2, labels.getString("main.tabbedPane.matchingTool"));
			menu.setTitleAt(3, labels.getString("main.tabbedPane.database"));
			menu.setTitleAt(4, labels.getString("main.tabbedPane.preferences"));
	...
	private void importButtonPressed() {
		...
		try {
			...
			if (intConfig.getImportFileName().trim().equals("")) {
				errorMessage(labels.getString("import.dialog.error.incompleteData"), 
						labels.getString("import.dialog.error.incompleteData.dataset"));
				return;
			}
			...
			if (config.getProject().getDatabase().getWorkspace().getImportWorkspace().trim().equals("")) {
				errorMessage(labels.getString("import.dialog.error.incompleteData"),
						labels.getString("common.dialog.error.incompleteData.workspace"));
				return;
			}
			...
			if (filter.isSetSimple() &&
				...
				errorMessage(labels.getString("import.dialog.error.incorrectData"), 
						labels.getString("common.dialog.error.incorrectData.gmlId"));
				return;
			}
			...
			if (filter.isSetComplex() &&
				...
				if (coStart == null || (!coEndValue.trim().equals("") && coEnd == null)) {
					errorMessage(labels.getString("import.dialog.error.incorrectData"), 
							labels.getString("import.dialog.error.incorrectData.range"));
					return;
				}
			...
			if (filter.isSetComplex() &&
				...
				errorMessage(labels.getString("import.dialog.error.incorrectData"),
						labels.getString("common.dialog.error.incorrectData.gmlName"));
				return;
			}
			...
			if (filter.isSetComplex() &&
				...
				if (xMin == null || yMin == null || xMax == null || yMax == null) {
					errorMessage(labels.getString("import.dialog.error.incorrectData"),
							labels.getString("common.dialog.error.incorrectData.bbox"));
					return;
				}
			}
			...
			final ImpExpStatusDialog importDialog = new ImpExpStatusDialog(this, 
					labels.getString("import.dialog.title"), 
					labels.getString("import.dialog.msg"), 
					eventDispatcher);
			...
			importDialog.getAbbrechenButton().addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					SwingUtilities.invokeLater(new Runnable() {
						public void run() {
							importDialog.getHeaderLabel().setText(labels.getString("common.dialog.msg.abort"));
	...
	private void validateButtonPressed() {
		...
		try {
			...
			if (config.getInternal().getImportFileName().trim().equals("")) {
				errorMessage(labels.getString("validate.dialog.error.incompleteData"),
						labels.getString("validate.dialog.error.incompleteData.dataset"));
				return;
			}

			statusText.setText(labels.getString("main.status.validate.label"));
			LOG.info("Initializing XML validation...");

			// initialize event dispatcher
			final EventDispatcher eventDispatcher = new EventDispatcher();
			final StatusDialog validatorDialog = new StatusDialog(this, 
					labels.getString("validate.dialog.title"), 
					labels.getString("validate.dialog.msg"), 
					labels.getString("validate.dialog.details") , true);

			SwingUtilities.invokeLater(new Runnable() {
				public void run() {
					validatorDialog.pack();
			...
			validatorDialog.getButton().addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					SwingUtilities.invokeLater(new Runnable() {
						public void run() {
							validatorDialog.getHeaderLabel().setText(labels.getString("common.dialog.msg.abort"));
							eventDispatcher.triggerEvent(new InterruptEvent(
									InterruptEnum.USER_ABORT, 
	...
	private void exportButtonPressed() {
		...
		try {
			...
			if (intConfig.getExportFileName().trim().equals("")) {
				errorMessage(labels.getString("export.dialog.error.incompleteData"), 
						labels.getString("export.dialog.error.incompleteData.dataset"));
				return;
			}
			...
			if (!db.getWorkspace().getExportDate().trim().equals("")) {
				...
				try {
					...
					errorMessage(labels.getString("export.dialog.error.incorrectData"), 
							labels.getString("export.dialog.error.incorrectData.date"));
					return;
				}
			...
			if (filter.isSetComplex() &&
				...
				if (coStart == null || (!coEndValue.trim().equals("") && coEnd == null)) {
					errorMessage(labels.getString("export.dialog.error.incorrectData"), 
							labels.getString("export.dialog.error.incorrectData.range"));
					return;
				}
			...
			// initialize event dispatcher
			final EventDispatcher eventDispatcher = new EventDispatcher();
			final ImpExpStatusDialog exportDialog = new ImpExpStatusDialog(this, 
					labels.getString("export.dialog.title"),
					labels.getString("export.dialog.msg"),
					eventDispatcher);
			...
			exportDialog.getAbbrechenButton().addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					SwingUtilities.invokeLater(new Runnable() {
						public void run() {
							exportDialog.getHeaderLabel().setText(labels.getString("common.dialog.msg.abort"));
							eventDispatcher.triggerEvent(new InterruptEvent(
									InterruptEnum.USER_ABORT, 
	...
	private void matchButtonPressed() {
		...
		try {
			...
			statusText.setText(labels.getString("main.status.match.label"));
			LOG.info("Initializing matching process...");
			
			// initialize event dispatcher
			final EventDispatcher eventDispatcher = new EventDispatcher();
			final StatusDialog status = new StatusDialog(this, 
					labels.getString("match.step1.dialog.title"), 
					labels.getString("match.step1.dialog.msg"),
					labels.getString("match.step1.dialog.details"),
					true);	
			...
			status.getButton().addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					SwingUtilities.invokeLater(new Runnable() {
						public void run() {
							status.getHeaderLabel().setText(labels.getString("common.dialog.msg.abort"));
							eventDispatcher.triggerEvent(new InterruptEvent(
									InterruptEnum.USER_ABORT, 
	...
	private void mergeButtonPressed() {
		...
		try {
			...
			statusText.setText(labels.getString("main.status.merge.label"));
			LOG.info("Initializing merging process...");
			
			// initialize event dispatcher
			final EventDispatcher eventDispatcher = new EventDispatcher();
			final StatusDialog status = new StatusDialog(this, 
					labels.getString("match.step2.dialog.title"), 
					labels.getString("match.step2.dialog.msg"),
					labels.getString("match.step2.dialog.details"),
					false);	
	...
	private void deleteButtonPressed() {
		...
		try {
			...
			statusText.setText(labels.getString("main.status.delete.label"));
			LOG.info("Initializing deletion of buildings...");
			
			String msg = labels.getString("match.tools.dialog.msg");
			...
			final EventDispatcher eventDispatcher = new EventDispatcher();
			final StatusDialog status = new StatusDialog(this, 
					labels.getString("match.tools.dialog.title"), 
					result,
					labels.getString("match.tools.dialog.details"),
					false);	
	...
	public boolean saveIniFile() {
		...
		if (configPath == null) {
			String text = labels.getString("common.dialog.error.io.configPath");
			Object[] args = new Object[]{ config.getInternal().getConfigPath() };
			String result = MessageFormat.format(text, args);

			errorMessage(labels.getString("common.dialog.error.io.title"), result);
			return false;
		}
		...
		try {
			...
			errorMessage(labels.getString("common.dialog.error.io.title"), 
					labels.getString("common.dialog.error.io.general"));
			return false;
		}
	...
	private void closeDBConnection() {
		if (dbPool != null) {
			try {
				...
				String text = labels.getString("db.dialog.error.closeConn");
				Object[] args = new Object[]{ e.getMessage() };
				String result = MessageFormat.format(text, args);

				errorMessage(labels.getString("common.dialog.error.db.title"), result);
			}
		}
	}
