...
public class MenuProject extends JMenu {
	...
	private void init() {
		...
		importProject.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (file != null) {
					LOG.info("Projekteinstellungen werden geladen von " + file.toString());
					...
					try {
						...
						if (project != null) {
							...
							LOG.error("Projekteinstellungen konnten nicht gelesen werden.");
					} catch (FileNotFoundException e1) {
						LOG.error("Projekteinstellungen konnten nicht gelesen werden: " + e1.getMessage());
		...
		exportProject.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (file != null) {
					LOG.info("Projekteinstellungen werden gesichert unter " + file.toString());
					try {
						((ImpExpGui)getTopLevelAncestor()).setSettings();
						ProjectConfigUtil.marshal(config.getProject(), file.toString(), ctx);
					} catch (JAXBException e1) {
						LOG.error("Projekteinstellungen konnten nicht gesichert werden: " + e1.getMessage());
					}
				}				
			}
		});
		...
		xsdProject.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (file != null) {
					LOG.info("Projekt XSD wird gesichert unter " + file.toString());
					try {
						ProjectConfigUtil.generateSchema(ctx, file);
					} catch (FileNotFoundException e1) {
						LOG.error("Projekt XSD konnte nicht gesichert werden: " + e1.getMessage());
	...
	public void doTranslation() {
		importProject.setText(ImpExpGui.labels.getString("menu.project.open.label"));
