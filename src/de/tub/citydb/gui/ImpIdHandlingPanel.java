...
import de.tub.citydb.config.project.importer.ImpGmlIdCodeSpaceMode;
import de.tub.citydb.config.project.importer.ImpUUIDMode;
...
public class ImpIdHandlingPanel extends PrefPanelBase {
	...
	//Konstruktor
	public ImpIdHandlingPanel(Config inpConfig) {
		super(inpConfig);
		initGui();
	}
	...
	public void doTranslation() {
		block2.setBorder(BorderFactory.createTitledBorder(ImpExpGui.labels.getString("pref.import.idHandling.border.idCodeSpace")));
		impIdCSRadioFile.setText(ImpExpGui.labels.getString("pref.import.idHandling.label.idCodeSpace.file"));
		impIdCSRadioFilePath.setText(ImpExpGui.labels.getString("pref.import.idHandling.label.idCodeSpace.filePath"));
		impIdCSRadioUser.setText(ImpExpGui.labels.getString("pref.import.idHandling.label.idCodeSpace.user"));
		block1.setBorder(BorderFactory.createTitledBorder(ImpExpGui.labels.getString("pref.import.idHandling.border.idAllocation")));
		impIdCheckExtRef.setText(ImpExpGui.labels.getString("pref.import.idHandling.label.idAllocation.extReference"));
		impIdRadioAdd.setText(ImpExpGui.labels.getString("pref.import.idHandling.label.idAllocation.add"));
		impIdRadioExchange.setText(ImpExpGui.labels.getString("pref.import.idHandling.label.idAllocation.exchange"));
	}
	...
	public void setSettings() {
		...
		if (impIdCSRadioFile.isSelected()) {
			gmlId.setCodeSpaceMode(ImpGmlIdCodeSpaceMode.RELATIVE);
		}
		else if (impIdCSRadioFilePath.isSelected()) {
			gmlId.setCodeSpaceMode(ImpGmlIdCodeSpaceMode.ABSOLUTE);
		}
		else {
			gmlId.setCodeSpaceMode(ImpGmlIdCodeSpaceMode.USER);
		}
		gmlId.setCodeSpace(impIdCSUserText.getText());

		if (impIdRadioAdd.isSelected()) {
			gmlId.setUuidMode(ImpUUIDMode.COMPLEMENT);
		}
		else {
			gmlId.setUuidMode(ImpUUIDMode.REPLACE);
		}
