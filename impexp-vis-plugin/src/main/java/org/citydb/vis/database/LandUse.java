...
public class LandUse extends AbstractVisObject {
	...
	protected Styles getStyles() {
		return config.getVisExportConfig().getLandUseStyles();
	}

	public Balloon getBalloonSettings() {
		return config.getVisExportConfig().getLandUseBalloon();
	}
	...
	public void read(DBSplittingResult work) {
		...
		try {
			...
			while (!found && currentLod >= minLod) {
				...
				try {
					String query = queries.getLandUseQuery(currentLod, work.getDisplayForm(), work.getObjectClassId());
