...
public class Building extends AbstractVisObject {
	...
	protected Styles getStyles() {
		return config.getVisExportConfig().getBuildingStyles();
	}

	public Balloon getBalloonSettings() {
		return config.getVisExportConfig().getBuildingBalloon();
	}
	...
	private List<PlacemarkType> readBuildingPart(long buildingPartId, DBSplittingResult work) {
		PreparedStatement psQuery = null;
		ResultSet rs = null;
		...
		try {
			...
			Lod0FootprintMode lod0FootprintMode = config.getVisExportConfig().getLod0FootprintMode();
