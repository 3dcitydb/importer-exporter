...
public class GenericCityObject extends AbstractVisObject {
	...
	protected Styles getStyles() {
		return config.getVisExportConfig().getGenericCityObjectStyles();
	}

	public Balloon getBalloonSettings() {
		if (isPointOrCurve) {
			if (isPoint)
				return config.getVisExportConfig().getGenericCityObjectPointAndCurve().getPointBalloon();
			else
				return config.getVisExportConfig().getGenericCityObjectPointAndCurve().getCurveBalloon();
		}
		// default
		return config.getVisExportConfig().getGenericCityObject3DBalloon();
	}
		...
		try {
			...
			if ((rs == null) || // result empty
				...
				if (isPointOrCurve) { // point or curve geometry
					visExporterManager.print(createPlacemarksForPointOrCurve(rs, work, config.getVisExportConfig().getGenericCityObjectPointAndCurve()),
							work,
