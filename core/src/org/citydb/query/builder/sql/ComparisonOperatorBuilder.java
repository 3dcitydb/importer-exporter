...
public class ComparisonOperatorBuilder {
	...
	private SQLQueryContext buildBinaryOperator(BinaryComparisonOperator operator, boolean negate) throws QueryBuildException {
		...
		// map operands
		vcs.sqlbuilder.expression.Expression rightOperand = literal.convertToSQLPlaceHolder();
		vcs.sqlbuilder.expression.Expression leftOperand = queryContext.targetColumn;
	...
	private SQLQueryContext buildLikeOperator(LikeOperator operator, boolean negate) throws QueryBuildException {
		...
		queryContext.select.addSelection(ComparisonFactory.like(queryContext.targetColumn, 
				...
				value.contains(escapeCharacter) ? new vcs.sqlbuilder.expression.StringLiteral(escapeCharacter) : null, 
						negate));
