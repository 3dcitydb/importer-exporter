...
import vcs.sqlbuilder.expression.AbstractSQLLiteral;
import vcs.sqlbuilder.expression.DoubleLiteral;
import vcs.sqlbuilder.expression.Expression;
import vcs.sqlbuilder.expression.IntegerLiteral;
...
import vcs.sqlbuilder.schema.AliasGenerator;
import vcs.sqlbuilder.schema.Column;
import vcs.sqlbuilder.schema.DefaultAliasGenerator;
import vcs.sqlbuilder.schema.GlobalAliasGenerator;
...
public class SchemaPathBuilder {
	...
	private void addJoin(Select select, AbstractJoin abstractJoin) throws QueryBuildException {		
		if (abstractJoin instanceof Join) {
			...
			ListIterator<vcs.sqlbuilder.select.join.Join> iter = select.getJoins().listIterator(select.getJoins().size());
			while (iter.hasPrevious()) {
				vcs.sqlbuilder.select.join.Join candidate = iter.previous();
