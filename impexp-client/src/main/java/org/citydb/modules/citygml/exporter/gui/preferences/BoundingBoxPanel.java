...
public class BoundingBoxPanel extends AbstractPreferencesComponent {
	...
	public boolean isModified() {
		...
		if (((Number)rowsText.getValue()).intValue() != tilingOptions.getRows()) return true;
		if (((Number)columnsText.getValue()).intValue() != tilingOptions.getColumns()) return true;

		if (!tilingOptions.getTilePath().equals(tilePathName.getText().trim())) return true;

		if ((TileSuffixMode)tilePathSuffixComboBox.getSelectedItem() != tilingOptions.getTilePathSuffix()) return true;
		if ((TileNameSuffixMode)tileNameSuffixComboBox.getSelectedItem() != tilingOptions.getTileNameSuffix()) return true;

		if (tilingOptions.isIncludeTileAsGenericAttribute() != setGenAttr.isSelected()) return true;
		if ((TileSuffixMode)genAttrValueComboBox.getSelectedItem() != tilingOptions.getGenericAttributeValue()) return true;
	...
	public void loadSettings() {
		...
		// tiling options
		TilingOptions tilingOptions = query.getTilingOptions();
		rowsText.setValue(tilingOptions.getRows());
		columnsText.setValue(tilingOptions.getColumns());
	...
	public void setSettings() {
		...
		tilingOptions.setRows(((Number)rowsText.getValue()).intValue());
		tilingOptions.setColumns(((Number)columnsText.getValue()).intValue());
	}
