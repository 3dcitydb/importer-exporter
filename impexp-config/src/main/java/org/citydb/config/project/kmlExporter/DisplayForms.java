...
public class DisplayForms {
    ...
    public boolean contains(DisplayFormType type) {
        return displayForms.containsKey(type);
    }
    ...
    public DisplayForm getOrDefault(DisplayFormType type) {
        return getOrDefault(type, false);
    }

    public DisplayForm getOrDefault(DisplayFormType type, boolean withSemanticSurfaces) {
        DisplayForm displayForm = displayForms.get(type);
        return displayForm != null ? displayForm : DisplayForm.of(type, withSemanticSurfaces);
    }

    public DisplayForm getOrDefault(DisplayFormType type, DisplayForm defaultValue) {
        return displayForms.getOrDefault(type, defaultValue);
    }

    public DisplayForm computeIfAbsent(DisplayFormType type, Function<? super DisplayFormType, ? extends DisplayForm> mappingFunction) {
        return displayForms.computeIfAbsent(type, mappingFunction);
    }
    ...
    public void copyFrom(DisplayForms other) {
        other.displayForms.forEach(displayForms::put);
    }
}
