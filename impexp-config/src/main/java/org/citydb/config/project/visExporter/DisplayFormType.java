...
@XmlType(name = "DisplayFormTypeType")
@XmlEnum
public enum DisplayFormType {
    @XmlEnumValue("footprint")
    FOOTPRINT("footprint", 0),
    @XmlEnumValue("extruded")
    EXTRUDED("extruded", 1),
    @XmlEnumValue("geometry")
    GEOMETRY("geometry", 1),
    @XmlEnumValue("collada")
    COLLADA("collada", 1);

    private final String name;
    private final int minimumLod;

    DisplayFormType(String name, int minimumLod) {
        this.name = name;
