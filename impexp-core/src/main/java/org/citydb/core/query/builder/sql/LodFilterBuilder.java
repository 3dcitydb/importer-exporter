    ...
    private List<LodFilterQueryContext> buildLodQueryContexts(FeatureType type, Table targetTable) {
        ...
        while (type != null) {
            for (AbstractProperty property : type.getProperties()) {
                if (property.isSetJoin() && property.getJoin() instanceof org.citydb.core.database.schema.mapping.Join) {
                    org.citydb.core.database.schema.mapping.Join join = (org.citydb.core.database.schema.mapping.Join) property.getJoin();
            ...
            if (type.isSetExtension() && type.getExtension().getBase() != cityObject) {
                ...
                if (extension.isSetJoin()) {
                    org.citydb.core.database.schema.mapping.Join join = extension.getJoin();
                    ...
                    if (subContext.hasLodPredicates()) {
                        ...
                        if (property instanceof InjectedProperty) {
                            ...
                            if (property.isSetJoin()) {
                                ...
                                if (abstractJoin instanceof org.citydb.core.database.schema.mapping.Join) {
                                    org.citydb.core.database.schema.mapping.Join join = (org.citydb.core.database.schema.mapping.Join) abstractJoin;
                            ...
                            org.citydb.core.database.schema.mapping.Join join = injectedProperty.getBaseJoin();
