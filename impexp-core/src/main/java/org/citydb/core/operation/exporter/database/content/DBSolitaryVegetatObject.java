...
public class DBSolitaryVegetatObject extends AbstractFeatureExporter<SolitaryVegetationObject> {
    ...
    public DBSolitaryVegetatObject(Connection connection, CityGMLExportManager exporter) throws CityGMLExportException, SQLException {
        ...
        if (projectionFilter.containsProperty("species", vegetationModule))
            select.addProjection(table.getColumn("species"), table.getColumn("species_codespace"));
        if (projectionFilter.containsProperty("height", vegetationModule))
            select.addProjection(table.getColumn("height"), table.getColumn("height_unit"));
        if (projectionFilter.containsProperty("trunkDiameter", vegetationModule))
            select.addProjection(table.getColumn("trunk_diameter"), table.getColumn("trunk_diameter_unit"));
        if (projectionFilter.containsProperty("crownDiameter", vegetationModule))
            select.addProjection(table.getColumn("crown_diameter"), table.getColumn("crown_diameter_unit"));
        if (lodFilter.isEnabled(1)) {
            if (projectionFilter.containsProperty("lod1Geometry", vegetationModule))
        ...
        if (lodFilter.isEnabled(2)) {
            if (projectionFilter.containsProperty("lod2Geometry", vegetationModule))
        ...
        if (lodFilter.isEnabled(3)) {
            if (projectionFilter.containsProperty("lod3Geometry", vegetationModule))
        ...
        if (lodFilter.isEnabled(4)) {
            if (projectionFilter.containsProperty("lod4Geometry", vegetationModule))
