...
public class DBLandUse extends AbstractFeatureExporter<LandUse> {
    ...
    public DBLandUse(Connection connection, CityGMLExportManager exporter) throws CityGMLExportException, SQLException {
        ...
        if (lodFilter.isEnabled(0) && projectionFilter.containsProperty("lod0MultiSurface", landUseModule))
            select.addProjection(table.getColumn("lod0_multi_surface_id"));
        if (lodFilter.isEnabled(1) && projectionFilter.containsProperty("lod1MultiSurface", landUseModule))
            select.addProjection(table.getColumn("lod1_multi_surface_id"));
        if (lodFilter.isEnabled(2) && projectionFilter.containsProperty("lod2MultiSurface", landUseModule))
            select.addProjection(table.getColumn("lod2_multi_surface_id"));
        if (lodFilter.isEnabled(3) && projectionFilter.containsProperty("lod3MultiSurface", landUseModule))
            select.addProjection(table.getColumn("lod3_multi_surface_id"));
        if (lodFilter.isEnabled(4) && projectionFilter.containsProperty("lod4MultiSurface", landUseModule))
