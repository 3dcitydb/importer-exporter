...
public class DBWaterBody extends AbstractFeatureExporter<WaterBody> {
    ...
    public DBWaterBody(Connection connection, CityGMLExportManager exporter) throws CityGMLExportException, SQLException {
        ...
        if (lodFilter.isEnabled(0)) {
            if (projectionFilter.containsProperty("lod0MultiCurve", waterBodyModule))
                select.addProjection(exporter.getGeometryColumn(table.getColumn("lod0_multi_curve")));
            if (projectionFilter.containsProperty("lod0MultiSurface", waterBodyModule))
                select.addProjection(table.getColumn("lod0_multi_surface_id"));
        }
        if (lodFilter.isEnabled(1)) {
            if (projectionFilter.containsProperty("lod1MultiCurve", waterBodyModule))
                select.addProjection(exporter.getGeometryColumn(table.getColumn("lod1_multi_curve")));
            if (projectionFilter.containsProperty("lod1MultiSurface", waterBodyModule))
                select.addProjection(table.getColumn("lod1_multi_surface_id"));
            if (projectionFilter.containsProperty("lod1Solid", waterBodyModule))
                select.addProjection(table.getColumn("lod1_solid_id"));
        }
        if (lodFilter.isEnabled(2) && projectionFilter.containsProperty("lod2Solid", waterBodyModule))
            select.addProjection(table.getColumn("lod2_solid_id"));
        if (lodFilter.isEnabled(3) && projectionFilter.containsProperty("lod3Solid", waterBodyModule))
            select.addProjection(table.getColumn("lod3_solid_id"));
        if (lodFilter.isEnabled(4) && projectionFilter.containsProperty("lod4Solid", waterBodyModule))
