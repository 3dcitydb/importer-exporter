...
STAT_FILE=../../SCHEMA/STATISTICS/STAT.sql
TMP_STAT_FILE=TMP_${SCHEMA_NAME}_STAT.sql

sed 's/'$TOKEN'/'$SCHEMA_NAME'/g' $DELETE_FILE > $TMP_DELETE_FILE
sed 's/'$TOKEN'/'$SCHEMA_NAME'/g' $ENVELOPE_FILE > $TMP_ENVELOPE_FILE
sed 's/'$TOKEN'/'$SCHEMA_NAME'/g' $STAT_FILE > $TMP_STAT_FILE
echo 'Done.'

# Run CREATE_SCHEMA.sql to create a new 3DCityDB schema -----------------------
echo
echo "Connecting to \"$PGUSER@$PGHOST:$PGPORT/$CITYDB\" ..."
psql -d "$CITYDB" -f "CREATE_SCHEMA.sql" -v schema_name="$SCHEMA_NAME" -v tmp_delete_file="$TMP_DELETE_FILE" -v tmp_envelope_file="$TMP_ENVELOPE_FILE" -v tmp_stat_file="$TMP_STAT_FILE"
