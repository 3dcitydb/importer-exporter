...
set STAT_FILE=..\..\SCHEMA\STATISTICS\STAT.sql
set TMP_STAT_FILE=TMP_%SCHEMA_NAME%_STAT.sql
...
(for /f "delims=" %%A in (%STAT_FILE%) do (
  set line=%%A
  setlocal enabledelayedexpansion
  echo !line:%TOKEN%=%SCHEMA_NAME%!
  endlocal
...
"%PGBIN%\psql" -d "%CITYDB%" -f "CREATE_SCHEMA.sql" -v schema_name="%SCHEMA_NAME%" -v tmp_delete_file="%TMP_DELETE_FILE%" -v tmp_envelope_file="%TMP_ENVELOPE_FILE%" -v tmp_stat_file="%TMP_STAT_FILE%"

:: Remove temporary SQL scripts -----------------------------------------------
del %TMP_DELETE_FILE% >nul 2>&1
del %TMP_ENVELOPE_FILE% >nul 2>&1
del %TMP_STAT_FILE% >nul 2>&1
