...
-- 1.0.0     2012-05-11   PostGIS version    TKol     FKun
--                                           APot
--
-------------------------------------------------------------------------------
-- Conversion-Report:
...
BEGIN
  ...
  SELECT ST_Evelope(ORTHOPHOTOPROPERTY) INTO fprnt 
     FROM ORTHOPHOTO WHERE id = orthophoto_id FOR UPDATE;
  ...
  -- // create pyramid
  -- Raster Overview, as they're called in the PostGIS world,
  -- are usually created during the raster2pgsql-process
  -- maybe a different approach has to be used here
  /*
...
BEGIN
  ...
  -- // update footprint
  SELECT ST_Envelope(RASTERPROPERTY) INTO fprnt FROM RASTER_RELIEF 
  ...
  */
  ...
  UPDATE RASTER_RELIEF_IMP SET RELIEF_ID=relief_id, RASTER_RELIEF_ID=relief_component_id;
