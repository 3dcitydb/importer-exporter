...
SELECT public.AddGeometryColumn('match_overlap_all', 'intersection_geometry', 3068, 'GEOMETRY', 3);

ALTER TABLE match_overlap_all(
	ADD COLUMN intersection_area INTEGER,
	ADD COLUMN area1_cov_by_area2 INTEGER,
	ADD COLUMN area2_cov_by_area1 INTEGER
	);
...
SELECT public.AddGeometryColumn('match_overlap_relevant', 'intersection_geometry', 3068, 'GEOMETRY', 3);

ALTER TABLE match_overlap_relevant(
      intersection_area INTEGER,
      area1_cov_by_area2 INTEGER,
      area2_cov_by_area1 INTEGER
	  );
...
SELECT public.AddGeometryColumn('match_master_projected', 'geometry', 3068, 'GEOMETRY', 3);
...
SELECT public.AddGeometryColumn('match_cand_projected', 'geometry', 3068, 'GEOMETRY', 3);
...
SELECT public.AddGeometryColumn('match_collect_geom', 'geometry', 3068, 'GEOMETRY', 3);  
...
AS 
  procedure create_matching_table(lod_cand INTEGER, lineage cityobject.lineage%type, lod_master INTEGER, delta_cand INTEGER, delta_master INTEGER, tolerance INTEGER := 0.001, aggregate_building INTEGER := 1);  
 
  procedure collect_cand_building(lod INTEGER, lineage cityobject.lineage%type);
  procedure collect_master_building(lod INTEGER, lineage cityobject.lineage%type);
  procedure collect_geometry(lod INTEGER);
  procedure rectify_geometry(tolerance INTEGER := 0.001);
  procedure aggregate_geometry(table_name VARCHAR, tolerance INTEGER := 0.001, aggregate_building INTEGER := 1);
  procedure join_cand_master(lod_cand INTEGER, lineage cityobject.lineage%type, lod_master INTEGER, tolerance INTEGER := 0.001);
	procedure create_relevant_matches(delta_cand INTEGER, delta_master INTEGER);
  procedure clear_matching_tables;
 
  function aggregate_mbr(table_name VARCHAR) return mdsys.sdo_geometry;
  function aggregate_geometry_by_id(id INTEGER, tolerance INTEGER := 0.001, aggregate_building INTEGER := 1) return mdsys.sdo_geometry; 
END geodb_match;
/
...
AS
  -- private procedures
  function get_2d_srid return INTEGER;
  ...
  function aggregate_mbr(table_name VARCHAR)
  ...
  * polygons.
  */
  function aggregate_geometry_by_id(id INTEGER, tolerance INTEGER := 0.001, aggregate_building INTEGER := 1)
  return mdsys.sdo_geometry
  is
