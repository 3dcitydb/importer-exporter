...
AS 
  procedure create_matching_table(lod_cand number, lineage cityobject.lineage%type, lod_master number, tolerance number := 0.001, aggregate_building number := 1);  
...
AS
  ...
  match_master_aggr_geom_spx index_obj := 
    index_obj.construct_spatial_3d('match_master_aggr_geom_spx', MASTER_GEOMETRY_TABLE, 'geometry'); 
  match_cand_aggr_geom_spx index_obj := 
    index_obj.construct_spatial_3d('match_cand_aggr_geom_spx', CAND_GEOMETRY_TABLE, 'geometry');    
  match_result_spx index_obj := 
    index_obj.construct_spatial_3d('match_result_spx', 'match_result', 'intersection_geometry');
  
  match_allocate_id_idx index_obj :=
    index_obj.construct_normal('match_allocate_id_idx', 'match_allocate_geom', 'id');
  match_allocate_root_id_idx index_obj :=
    index_obj.construct_normal('match_allocate_root_id_idx', 'match_allocate_geom', 'root_id');
  ...
  begin
    ...
    execute immediate 'update match_allocate_geom set geometry = sdo_util.rectify_geometry(geometry, :1) 
        where sdo_geom.validate_geometry(geometry, :2) <> ''TRUE''' using tolerance, tolerance;
    ...
    execute immediate 'delete from match_allocate_geom where sdo_geom.sdo_area(geometry, :1) <= 0' using tolerance;
  exception
    when others then
  ...
  begin
    ...
    execute immediate 'insert all /*+ append nologging */ into match_result 
      ...
      execute immediate 'update match_result set intersection_area = sdo_geom.sdo_area(intersection_geometry, :1)' using tolerance;
      execute immediate 'delete from match_result where intersection_area = 0';
      execute immediate 'update match_result set area1_cov_by_area2 = geodb_util.min(intersection_area / area1, 1.0), 
